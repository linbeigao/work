    ...
    public void draw() {
        ...
        emotion_1.ants.stream()
                ...
                .forEach(ant -> {
                    ...
                    strokeWeight(2);
                    beginShape();
                    curveVertex((x-random(0,50)-100)/4+460,(y-15)/2);
                    curveVertex((x*3-random(40,100))/2+460,y/3);
                    curveVertex((x-y)/7+460,(x-2*y)/9);
                    curveVertex(5*m/12+460,r*r2/600);
                    curveVertex((x-2*y)/2+460,5*m/13);
                    curveVertex(m/2+460,r/7);
                    curveVertex(3*x/5+460,101*y/199);
                    curveVertex(x-x1+460,x2/2+random(-70,70));
                    curveVertex(size1/2+460,size2/2);
                    endShape();
                    
                    strokeWeight(2);
                    beginShape();
                    curveVertex(size1-r-random(-3,5)+460,size2-r2*sin(theta));
                    curveVertex((random(100,400)-x/12)/2+460,y/3-r2/46);
                    curveVertex(size1/4-min(r/44,r2/27,abs(random(0,345)-r))+460,size2/3+random(-15,10)+ceil(r2));
                    curveVertex(800-5*m/12+460,r-r*r2/600);
                    curveVertex(150-min(size1/3,r/7,x1-x)+460,150+random(-5,20));
                    endShape();
                    
                    noFill();
                    strokeWeight(1);
                    beginShape();
                    curveVertex(x1/2-m/1000+460,y/2+random(0,7));
                    curveVertex(x*sin(this.theta)/6+460,y/20+random(0,40));
                    curveVertex(floor(x/103)+460,y/88);
                    curveVertex((x1+x2)/55-r2/107+460,ceil(y/8-r-random(0,75)));
                    curveVertex(2*size1/3+460,3*size2/4);
                    curveVertex(2*width/3-460+random(0,500),3*size2/4-random(0,250)-r/16);
                    curveVertex(99*width/101,22*height/23-x1/12+x2/100);
                    endShape();
                    
                    strokeWeight(1);
                    beginShape();
                    curveVertex(size1-random(0,300)+460,size2*7/9-r/20);
                    curveVertex(size1/2-r+460,m-pow(r2,2)/14);
                    curveVertex(size1/4-r+460,m-pow(r2,3)/37);
                    curveVertex(ceil((size1-random(0,70))/3)-r2+r+460,m/4);
                    curveVertex(x1+460,x);
                    curveVertex(y-r-r2+460,x+y-2*r);
                    endShape();
        ...
        emotion_2.ants.stream()
                ...
                .forEach(ant -> {
                    ...
                    strokeWeight(1);
                    beginShape();
                    curveVertex(size1*5/6-random(-70,70)+460, size2*8/9-random(-50,50));
                    curveVertex(size1*7/9-x3+460,size2*7/9-x4);
                    curveVertex(size1*7/9-x5/3+460,size2*7/18-ceil(x5/3-random(-50,50)));
                    curveVertex(size1*5/9+x3*x4+460,size2*5/9+x3*x4*x5);
                    curveVertex(ceil((size1-z))+460, 7*w/4);
                    curveVertex(k*(k+1)+460,x5/theta);
                    endShape();
                });
        emotion_3.ants.stream()
                .filter(not(HarassmentCurve::isCompleted))
                .forEach(ant -> {
                    var please = ant.getCurrentTrace().getDimension("z");
                    var a = amaze.get(ant);
                    if ("SMALL".equals(please.getName())){
                       a -= 3*move_amount/2;
                    }
                    if ("BIG".equals(please.getName())){
                       a += 3*move_amount/2;
                    }
                    amaze.replace(ant, a);
                    ...
                    strokeWeight(1);
                    beginShape();
                    curveVertex(size1/3*sin(PI/3+random(-2,2))+460,size2/3*cos(PI/3+random(-2,2)));
                    curveVertex(size1/3+s*sin(PI/3+random(-2,2))+460,size2/3+t*cos(PI/3+random(-2,2)));
                    curveVertex(size1/3+s*sin(PI*2/3)+460,size2/3+t*cos(PI*2/3));
                    curveVertex(size1*sin(PI*4/3+random(-2,2))+460,size2*cos(PI*4/3+random(-2,2)));
                    curveVertex(size1*sin(PI*5/3)+460,size2*cos(PI*5/3));
                    curveVertex(width*sin(PI*5/3)+460,size2*cos(PI*5/3)-height/5);
                    endShape();
                });
